...
const FeaturesItem = ({ feature }: { feature: { icon: React.ReactNode; title: string; image: string; descriptions: string[]; iconDescription: React.ReactNode; className: string } }) => {
  return (
    <div className='z-30 flex flex-col gap-2 rounded-3xl bg-white p-4 shadow-[0px_12px_24px_0px_#0000001F] sm:p-6 lg:translate-y-[80%]'>
      ...
      <Image src={feature.image} alt={feature.title} width={500} height={500} className='w-full' />
...
export default function AIAssistant() {
  ...
  return (
    <div className='relative min-h-[150dvh] bg-[#FFF7EB]'>
      <div className='absolute top-[12%] left-0 z-[1] h-dvw w-full'>
        <Image src='/feature.png' alt='logo' width={3000} height={1000} className='w-full' />
      </div>
      <div className='z-10 container mx-auto max-w-7xl px-4 py-8 sm:py-12 md:py-16'>
        <div className='mb-6 flex flex-col items-center gap-2 text-center sm:mb-8 sm:gap-3 md:mb-12 md:gap-4 lg:mb-16'>
          <Title title='Tính năng nổi bật' />
          <h1 className='z-30 mb-2 text-xl font-bold text-[#333333] sm:text-2xl md:text-3xl lg:text-6xl'>Vinimex AI đang xây dưng gì?</h1>
